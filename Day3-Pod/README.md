...
We have two methods to create pods or any object in kubernetes, i.e. imperative way and declarative way.
...
How to run the above mentioned sample yaml manifest. Save the file as pod.yaml
...
How to delete this pod 
```bash
kubectl delete pod/mypod
```
How to access a pod or get into a pod
```bash
kubectl exec -it pod/mypod
```

How to do a prot-forward for a pod and run it in browser
```bash
...
## Multi-container pod
We can create two or more than two containers in a pod as well, if required.
...
What if, you run this exec command without container name
```bash
